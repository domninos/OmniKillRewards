...
public class SetSubCommand implements SubCommand {
    ...
    public SetSubCommand(KillRewardsPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean execute(Player player, String label, String[] args) {
        ...
        switch (type) {
            ...
            case KILLS: {
                ...
                plugin.getCacheUtil().setKills(target, (int) amount);
                int newKills = plugin.getCacheUtil().getKills(target);

                String parsed = plugin.getMessagesUtil().getSetKills()
            ...
            case DEATHS: {
                ...
                plugin.getCacheUtil().setDeaths(target, (int) amount);
                int newDeaths = plugin.getCacheUtil().getDeaths(target);

                String parsed = plugin.getMessagesUtil().getSetDeaths()
                        .replace("%amount%", String.valueOf(amount))
                        .replace("%player%", target.getName())
                        .replace("%before%", String.valueOf(before))
            ...
            case KDR: {
                ...
                plugin.getCacheUtil().setKDR(target, amount);
                double newKDR = plugin.getCacheUtil().getKDR(target);

                String parsed = plugin.getMessagesUtil().getSetKDR()
                        .replace("%amount%", String.valueOf(amount))
                        .replace("%player%", target.getName())
                        .replace("%before%", String.valueOf(before))
            ...
            case KILLSTREAK: {
                ...
                plugin.getCacheUtil().setKillStreak(target, (int) amount);
                int newKillStreak = plugin.getCacheUtil().getKillStreak(target);

                String parsed = plugin.getMessagesUtil().getSetKillStreak()
                        .replace("%amount%", String.valueOf(amount))
                        .replace("%player%", target.getName())
                        .replace("%before%", String.valueOf(before))
    ...
    public String getUsage() {
        return "&cUsage: /killrewards set <type> <player> <amount>";
    }

    @Override
    public String getPermission() {
        return "killrewards.set";
    }
}
